...
#include "BouncingBallFlecsScene.h"
...
void BouncingBallFlecsScene::Initialize()
{
    ...
    GetWorld()
        .entity("GravitySettings")
        .set<GravitySettings>(
            {.gravity = PhysicsConstants::GRAVITY_DOWN, .pixelsPerCentimeter = PhysicsConstants::PIXELS_PER_CENTIMETER}
        );
    ...
    // --- Add systems ---
    GetWorld().system<Transform, Velocity, RigidBody>().each(ProcessPhysics);
    GetWorld().system<Transform, Velocity>().each(ProcessScreenBounce);
}

void BouncingBallFlecsScene::HandleEvent(const std::optional<sf::Event>& event, sf::RenderWindow& window)
{
...
void BouncingBallFlecsScene::Update(const float deltaTime)
{
    if (!IsLoaded() || IsPaused())
        return;

    // Set delta time and run systems
    GetWorld().progress(deltaTime);
}

void BouncingBallFlecsScene::Render(sf::RenderWindow& window)
{
    if (!IsLoaded())
        return;
...
void BouncingBallFlecsScene::CreateBalls(const int count)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        GetWorld()
            ...
            .set<RigidBody>({.damping = PhysicsConstants::DEFAULT_DAMPING})
            .set<BallRenderable>({std::move(ballShape)});
    }
}

flecs::world& BouncingBallFlecsScene::GetWorld()
{
    return _world;
}

void BouncingBallFlecsScene::ProcessScreenBounce(const flecs::iter& it, size_t, Transform& t, Velocity& v)
{
    if (t.position.x < 0.f)
    {
        v.velocity.x *= -1.f;
...
void BouncingBallFlecsScene::ProcessPhysics(const flecs::iter& it, size_t, Transform& t, Velocity& v, const RigidBody& p)
{
    // Physics system
    if (const auto [gravity, pixelsPerCentimeter, enabled] = it.world().get<GravitySettings>(); enabled) {
